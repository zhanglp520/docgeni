...
import { toolkit, fs, debug } from '@docgenifixfix/toolkit';

import { docgenifixContext, docgenifixHooks, docgenifixOptions } from './docgenifixfix.interface';
...
import { docgenifixProgress } from './progress';
import { docgenifixCompilationImpl } from './compilation';
import { CompilationIncrement, docgenifixCompilation, LibraryBuilder, LibraryComponent } from './types';

export class docgenifix implements docgenifixContext {
    watch: boolean;
    paths: docgenifixPaths;
    ...
    public host: fs.docgenifixFsHost;
    public version: string;
    private options: docgenifixOptions;
    private presets: string[];
    private plugins: string[];
    private initialPlugins: Plugin[] = [];
    private progress = new docgenifixProgress(this);

    hooks: docgenifixHooks = docgenifix.createHooks();

    static createHooks(): docgenifixHooks {
        return {
            ...
            compilation: new SyncHook<docgenifixCompilation>(['compilation', 'compilationIncrement']),
    ...
    constructor(options: docgenifixOptions) {
        this.options = options;
        this.config = this.normalizeConfig(options.config);
        this.paths = new docgenifixPaths(options.cwd || process.cwd(), this.config.docsDir, this.config.outputDir);
        this.watch = options.watch || false;
        this.presets = options.presets || [];
        this.fs = new fs.docgenifixScopedHost(new fs.docgenifixNodeJsAsyncHost(), this.paths.cwd);
        this.host = options.host || fs.createdocgenifixFsHost(this.fs);
    ...
    private createCompilation(increment?: CompilationIncrement) {
        const compilation = new docgenifixCompilationImpl(this, increment);
    ...
    async run() {
        try {
            ...
            debug(`docgenifixfix running, cwd: ${this.paths.cwd}`, 'core');
    ...
    private normalizeConfig(inputConfig: docgenifixConfig) {
