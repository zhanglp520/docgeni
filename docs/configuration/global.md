...
文档的标题，通常为类库名，比如`docgenifix`。

## logoUrl
- 类型：`string`
- 默认：docgenifix 的 Logo Url
...
Markdown 文档目录地址，docgenifix 会扫描该目录下的文件夹和 Markdown 文件，按照一定的规则生成频道、菜单和页面文档。

## siteDir
- 类型：`string`
- 默认：`.docgenifixfix/site`

自动生成的站点目录，docgenifix 会把生成的组件示例和文档拷贝到该站点下，当`siteProjectName`设置后以自定义站点的所在目录为主，此配置无效。

## outputDir
- 类型：`string`
- 默认：`dist/docgenifixfix-site`

站点构建输出目录，当`siteProjectName`设置后以自定义站点的输出目录为主，此配置无效。

## publicDir
- 类型：`string`
- 默认：`.docgenifixfix/public`

文档站点的配置目录，docgenifix会把该文件夹下的`index.html`、`favicon.ico`、`styles.scss`、`assets`、`.browserslistrc`和`tsconfig.json`文件拷贝并覆盖站点目录实现自定义配置功能，更多配置参考 [自定义站点](/guides/advance/customize)。

## componentsDir <label>1.1.0+</label>
- 类型：`string`
- 默认：`.docgenifixfix/components`
...
Angular 自定义站点的项目名称，组件库开发除了默认的文档和示例展示功能外，可能还需要做一些自定义的功能，那么可以在仓储中新建一个 site 项目，然后配置此项目的名字，docgenifix 会把生成的文档和示例拷贝到该项目下，自定义站点的更多配置参考 [自定义站点](/guides/advance/customize)。
