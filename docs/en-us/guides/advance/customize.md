...
Docgeni will generate a documentation site in the `.docgenifix/site` directory by default. This site is entirely an Angular project. The directory structure is as follows:
```
.docgenifix
└── site
    ├── src
...
Sometimes it is necessary to import a third-party module and providers in the automatically generated `AppModule`. Docgeni allows user to define a `module.ts` under the `.docgenifix/app` folder, and then customize some metadata through the`export default { imports: [], providers: [] }` syntax.
...
Finally, you can execute command `docgenifix serve --port 4600` to visit the site.
